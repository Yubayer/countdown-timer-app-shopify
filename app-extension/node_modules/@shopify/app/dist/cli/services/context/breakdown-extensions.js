...
export async function extensionsIdentifiersDeployBreakdown(options) {
    ...
    if (options.release) {
        extensionIdentifiersBreakdown = await resolveRemoteExtensionIdentifiersBreakdown(options.developerPlatformClient, options.appId, extensionsToConfirm.validMatches, extensionsToConfirm.extensionsToCreate, extensionsToConfirm.dashboardOnlyExtensions);
    }
    return {
...
export async function extensionsIdentifiersReleaseBreakdown(token, apiKey, version) {
...
export async function configExtensionsIdentifiersBreakdown({ developerPlatformClient, apiKey, localApp, versionAppModules, release, }) {
...
async function resolveRemoteConfigExtensionIdentifiersBreakdown(developerPlatformClient, apiKey, app, versionAppModules) {
    const remoteConfig = await fetchAppRemoteConfiguration(apiKey, developerPlatformClient, app.specifications ?? [], app.remoteBetaFlags);
...
async function resolveRemoteExtensionIdentifiersBreakdown(developerPlatformClient, apiKey, localRegistration, toCreate, dashboardOnly) {
...
function loadExtensionsIdentifiersBreakdown(activeAppVersion, localRegistration, toCreate) {
    const extensionModules = activeAppVersion.app.activeAppVersion?.appModuleVersions.filter((module) => !module.specification || module.specification.experience === 'extension') || [];
...
function loadDashboardIdentifiersBreakdown(currentRegistrations, activeAppVersion) {
    const currentVersions = activeAppVersion.app.activeAppVersion?.appModuleVersions.filter((module) => module.specification.options.managementExperience === 'dashboard') || [];
